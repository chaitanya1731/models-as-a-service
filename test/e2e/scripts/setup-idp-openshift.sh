...
grant_user_roles() {
  ...
  log_info "Adding 'edit' cluster role to $NUM_USERS users..."
  
  local success=0
  local failed=0
  
  for ((i=1; i<=NUM_USERS; i++)); do
    if oc adm policy add-cluster-role-to-user edit "testuser-${i}" &>/dev/null; then
      ((success++))
    else
      ((failed++))
  ...
  if [[ $failed -eq 0 ]]; then
    log_success "Granted edit role to all $success users"
  else
